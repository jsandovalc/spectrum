...
- **Catch known git failures before they happen**: Validate preconditions before shelling out to git. For example, check for ref conflicts (a branch that would collide with a branch-as-prefix) before `git checkout -b`, rather than letting git fail with a cryptic `cannot lock ref` error. When a failure *is* predictable, raise a clear `click.ClickException` with context and recovery steps.
